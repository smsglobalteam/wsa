...
function gridColumns(span?: number | 'full') {
  if (span === 'full' || (span && span > 6 && smallerThanMd))
...
<template>
  <template v-for="[path, item] in Object.entries(schema)" :key="path">
    <n-form-item v-if="item.type !== 'space'" class="shrink-0" :label="item.label" :path="path" :rule="item.rules" :show-feedback="item.showFeedback" :show-label="item.type !== 'checkbox'" :style="gridColumns(item.span)">
      ...
      <n-select v-if="item.type === 'select'" v-model:value="modelValue![path]" clearable filterable :options="item.options" :placeholder="item.placeholder || ''" />
